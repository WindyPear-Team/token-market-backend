...
// RedeemCodeAPI handles balance redeem codes.
...
func (api *RedeemCodeAPI) List(c *gin.Context) {
	var codes []model.RedeemCode
	if err := model.DB.Preload("Group").Order("created_at DESC").Find(&codes).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (api *RedeemCodeAPI) Create(c *gin.Context) {
	var input redeemCodeInput
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	if input.Amount.IsZero() && (input.GroupID == nil || *input.GroupID == 0) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Amount or group is required"})
		return
	}
	...
	expiresAt, err := parseRedeemCodeExpiresAt(input.ExpiresAt)
	if err != nil {
	...
	redeemCode := model.RedeemCode{
		Code:              code,
		Amount:            input.Amount,
		GroupID:           redeemCodeGroupID(input.GroupID),
	...
	if err := model.DB.Create(&redeemCode).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, redeemCode)
}

func (api *RedeemCodeAPI) Update(c *gin.Context) {
	var redeemCode model.RedeemCode
	if err := model.DB.First(&redeemCode, c.Param("id")).Error; err != nil {
	...
	updates := map[string]interface{}{
		"amount":              input.Amount,
		"group_id":            redeemCodeGroupID(input.GroupID),
		"group_duration_days": input.GroupDurationDays,
		"allow_stacking":      input.AllowStacking,
		"max_uses":            maxUses,
		"enabled":             enabled,
		"expires_at":          expiresAt,
	}
	if code := normalizeRedeemCode(input.Code); code != "" {
		updates["code"] = code
	}
	if err := model.DB.Model(&redeemCode).Updates(updates).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if err := model.DB.First(&redeemCode, redeemCode.ID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, redeemCode)
...
func (api *RedeemCodeAPI) Delete(c *gin.Context) {
	if err := model.DB.Delete(&model.RedeemCode{}, c.Param("id")).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (api *RedeemCodeAPI) Redeem(c *gin.Context) {
	...
	if err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		var existing int64
		if err := tx.Model(&model.RedeemCodeRedemption{}).
			Where("redeem_code_id = ? AND user_id = ?", redeemed.ID, user.ID).
		...
		if err := tx.Create(&model.RedeemCodeRedemption{
		...
		if err := tx.Model(&model.User{}).
			Where("id = ?", user.ID).
			UpdateColumn("balance", gorm.Expr("balance + ?", redeemed.Amount)).Error; err != nil {
			return err
		}
		if redeemed.GroupID != nil && *redeemed.GroupID != 0 {
			if err := applyRedeemCodeGroupGrant(tx, user.ID, *redeemed.GroupID, redeemed.GroupDurationDays, redeemed.AllowStacking); err != nil {
				return err
			}
		}
	...
	if err := model.DB.Preload("Group").First(&refreshedUser, user.ID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func parseRedeemCodeExpiresAt(value string) (*time.Time, error) {
...
func redeemCodeGroupID(groupID *uint) *uint {
...
func applyRedeemCodeGroupGrant(tx *gorm.DB, userID uint, groupID uint, durationDays int, allowStacking bool) error {
	var group model.Group
	...
	return tx.Create(&model.UserGroupMembership{
		UserID:    userID,
		GroupID:   groupID,
		ExpiresAt: expiresAt,
	}).Error
